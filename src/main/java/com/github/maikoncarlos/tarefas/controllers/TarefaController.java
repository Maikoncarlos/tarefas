...
public class TarefaController {
    ...
    @PostMapping("/task")
    public ResponseEntity<String> createTask(@RequestBody TarefaDTO tarefaDTO) {
    ...
    @GetMapping("/task/{id}")
    public TarefaDTO getTasksToId(@PathVariable(value = "id") long id) {
        return tarefaService.getTasksToId(id);
    }

    @Operation(summary = "Atualiza uma Tarefa")
    @PutMapping("/task/{id}")
    public ResponseEntity<TarefaDTO> updateTask(@PathVariable(value = "id") long id, @RequestBody @Valid Tarefa tarefa) {
